...
lowerAndGreater n list = show n ++ " is greater than " ++ show ls ++ " elements and lower than " ++ show gs ++ " elements"
  where
    (ls, gs) = countLowsGreats n list 0 0

countLowsGreats :: Int -> [Int] -> Int -> Int -> (Int, Int)
countLowsGreats _ [] lows greats = (lows, greats)
countLowsGreats num (x:xs) lows greats
  | num > x   = countLowsGreats num xs (lows + 1) greats
  | num < x   = countLowsGreats num xs lows (greats + 1)
  | otherwise = countLowsGreats num xs lows greats