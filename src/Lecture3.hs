...
daysTo from to
  ...
  | otherwise = diff
  where
    diff = fromEnum to - fromEnum from
...
appendDiff3 x y z = res
  where
    helper' :: (Eq a, Monoid a) => a -> (a, [a]) -> (a, [a])
    helper' a (acc, sets)
      | a `elem` sets = (acc, sets)
      | otherwise = (acc <> a, a : sets)

    (acc', sets') = helper' x (mempty, [])
    (acc1, sets1) = helper' y (acc', sets')
    (res, _) = helper' z (acc1, sets1)
...
instance Foldable List1 where
  foldMap f (List1 x xs)  = f x `mappend` foldMap f xs
